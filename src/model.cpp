...
int model::init_model_parameters()
{
	...
	nl.resize(numSentiLabs); // k: for tying-JST model, add nl, the number of times that a word is associated with sentiment label l
	for (int l = 0; l < numSentiLabs; l++) {
	  nl[l] = 0;
	}
	...
	// k: tying-JST model change from theta_dlz to theta_lz
	theta_lz.resize(numSentiLabs);
	for (int l = 0; l < numSentiLabs; l++) {
	  theta_lz[l].resize(numTopics);
	}
...
// k: add compute tying-jst model's theta, instead of T*S*D. theta in tying-jst has T*S dimension.
void model::compute_theta_lz() {
  // k: nl is the total number of times that a word is associated with sentiment label l
  
  for (int l = 0; l < numSentiLabs; l++) {
    for (int z = 0; z < numTopics; z++) {
      theta_lz[l][z] = (nlz[l][z] + alpha_lz[l][z]) / (nl[l] + alphaSum_l[l]);
    }
  }
}
...
int model::save_model(string model_name) {
	...
	// k: save theta_lz for typing_JST model
	if (save_model_theta_lz(result_dir + model_name + theta_suffix)) 
	        return 1;
	return 0;
}
...
// k: save theta_lz for typing-JST model 
int model::save_model_theta_lz(string filename) {
  ...
  FILE * fout = fopen(filename.c_str(), "w");
    ...
    for (int l = 0; l < numSentiLabs; l++) {
      for (int z = 0; z < numTopics; z++) {
	fprintf(fout, "Label:%d  Topic:%d  %.15f\n", l, z, theta_lz[l][z]);
      }
      fprintf(fout, "\n");
    }
    // }
...
int model::init_estimate() {
  ...
  int sentiLab, topic; // k: sentiLab is how many sentiment label overall is pre-set, default is 3 bias is 2; topics is how many topics is pre-set
	srand(time(0)); // initialize for random number generation
	z.resize(numDocs); // k: numDocs is how many documents in the corpus; z is the vector stores every doc's each words' topic z
	l.resize(numDocs); // k: l is the vector stores every doc's each word's label

	for (int m = 0; m < numDocs; m++) {
		int docLength = pdataset->pdocs[m]->length;
		z[m].resize(docLength); // k: docLength is doc m's length, z stores every word's topic
		l[m].resize(docLength); // k: l stores every word's sentiment label
        ...
        for (int t = 0; t < docLength; t++) {
			...
			nlz[sentiLab][topic]++;
			nl[sentiLab]++; // k: add for typing-JST
        }
    }
...
int model::estimate() {
	...
	for (liter = 1; liter <= niters; liter++) {
		...
		if (savestep > 0 && liter % savestep == 0) {
			...
			compute_pi_dl();
			// compute_theta_dlz();
			compute_theta_lz(); // k: compute theta_lz for tying-JST model
			compute_phi_lzw();
...
int model::sampling(int m, int n, int& sentiLab, int& topic) {
	...
	nlzw[sentiLab][topic][pdataset->pdocs[m]->words[n]]++;
	nlz[sentiLab][topic]++;
	nl[sentiLab]++; // k: update word n's assignment of sentiment label 
